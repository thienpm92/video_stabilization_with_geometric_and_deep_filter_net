...
def main():
    ...
    with torch.no_grad():
        for idx in range(Nframe-1):
            ...
            Ip1 = Ip1.unsqueeze(0).float().cuda()
            Ip2 = Ip2.unsqueeze(0).float().cuda()
            
            batch = {'source_image':Ip1,'target_image':Ip2} 
            theta = geometric_model(batch)

            H = theta.reshape(2,3).cpu().numpy()
            A = H[0:2,0:2]
            t = H[0:2,2:3]
            orgTransform[idx,:] = np.hstack((A,t))
            if idx==0:
    ...
    for idx in range(Nstep):
        ...
        if idx == 0:    #first window
            ...
            smoothPath[startFrame:startFrame+overlap,0] = tmpA_
            smoothPath[startFrame:startFrame+overlap,1] = tmpB_
            smoothPath[startFrame:startFrame+overlap,2] = tmpX_
            smoothPath[startFrame:startFrame+overlap,3] = tmpC_
            smoothPath[startFrame:startFrame+overlap,4] = tmpD_
            smoothPath[startFrame:startFrame+overlap,5] = tmpY_

            smoothPath[startFrame+overlap:startFrame+winSize-overlap,0] = out_A[overlap:winSize-overlap]
            smoothPath[startFrame+overlap:startFrame+winSize-overlap,1] = out_B[overlap:winSize-overlap]
            smoothPath[startFrame+overlap:startFrame+winSize-overlap,2] = out_X[overlap:winSize-overlap]
            smoothPath[startFrame+overlap:startFrame+winSize-overlap,3] = out_C[overlap:winSize-overlap]
            smoothPath[startFrame+overlap:startFrame+winSize-overlap,4] = out_D[overlap:winSize-overlap]
            smoothPath[startFrame+overlap:startFrame+winSize-overlap,5] = out_Y[overlap:winSize-overlap]
    ...
    for idx in range(Nframe-1):
        ...
        newImg = cv2.warpAffine(img,param,imgSize)
    ...
    plt.plot(t,cameraPath[:,0,2])
    plt.plot(t,smoothPath[:,0,2])
    plt.show()
